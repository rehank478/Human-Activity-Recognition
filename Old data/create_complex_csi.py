...
if __name__ == "__main__":
    ...
    fig, axs = plt.subplots(3,figsize=(100,60))
    ...
    for i in range(int(len(Y[0]))):
        ...
        den = wden(amp,'heursure','soft','sln',5,'sym6')
        axs[2].plot(num,den,linewidth=0.5)
    ...
    for i in range(int(len(amplitude[0]))):
        ...
        for k in range(int(len(amplitude))):
            amp.append(amplitude[k][i])
            num.append(k)
        axs[1].plot(num,amp,linewidth=0.5)
    for ax in axs.flat:
        ax.set(xlabel='Packet No.', ylabel='CSI Amplitude(dB)')
    fig.show()
    fig.savefig(sr+'(DWT)''.png',dpi=200)
    ...
    #     freqs, psd = signal.welch(amp)
    #     plt.plot(freqs,psd,linewidth=0.5)
    # plt.xlabel('Packet No.')
